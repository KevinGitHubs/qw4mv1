...
// Data paket dengan bonus (contoh)
const packs = [
    { id: 'mlbb_86', game: 'mlbb', diamonds: 86, price: 20000, bonus: 0 },
    { id: 'mlbb_172', game: 'mlbb', diamonds: 172, price: 40000, bonus: 10 },
    { id: 'mlbb_257', game: 'mlbb', diamonds: 257, price: 60000, bonus: 20 },
    { id: 'ff_70', game: 'ff', diamonds: 70, price: 15000, bonus: 0 },
    { id: 'ff_140', game: 'ff', diamonds: 140, price: 30000, bonus: 5 },
    { id: 'ff_355', game: 'ff', diamonds: 355, price: 70000, bonus: 15 },
    { id: 'pubg_60', game: 'pubg', diamonds: 60, price: 18000, bonus: 0 },
    { id: 'pubg_180', game: 'pubg', diamonds: 180, price: 50000, bonus: 10 },
    { id: 'valorant_475', game: 'valorant', diamonds: 475, price: 95000, bonus: 25 },
];

const payments = [
    { id: 'bca', name: 'BCA VA', icon: 'fas fa-university' },
...
games.forEach(game => {
    ...
    card.innerHTML = `<i class="${game.icon}"></i><h3>${game.name}</h3>`;
...
payments.forEach(pay => {
    ...
    el.dataset.id = pay.id;
    el.innerHTML = `<i class="${pay.icon}"></i><span>${pay.name}</span>`;
...
function selectGame(game) {
    ...
    const selectedCard = Array.from(document.querySelectorAll('.game-card')).find(c => c.dataset.game === game.id);
...
function renderPacks(gameId) {
    ...
    filtered.forEach(pack => {
        ...
        const bonusText = pack.bonus > 0 ? `<div class="bonus">+${pack.bonus} Bonus!</div>` : '';
        packEl.innerHTML = `
...
function cekAkun() {
    ...
    accountNameDiv.style.display = 'flex';
    ...
    cekTimeout = setTimeout(async () => {
        try {
            const response = await fetch('/.netlify/functions/cek', {
                ...
                body: JSON.stringify({ game: selectedGame.id, userId, zoneId, server })
            });
            const data = await response.json();
            if (response.ok && data.success) {
                accountNameDiv.className = 'account-name success';
                accountNameDiv.innerHTML = `<i class="fas fa-check-circle"></i> Nama Akun: ${data.nickname}`;
            } else {
                accountNameDiv.className = 'account-name error';
                accountNameDiv.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${data.error || 'Akun tidak ditemukan'}`;
            }
        } catch (err) {
            accountNameDiv.className = 'account-name error';
            accountNameDiv.innerHTML = '<i class="fas fa-exclamation-circle"></i> Gagal menghubungi server';
        }
    }, 500);
}
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        submitBtn.innerHTML = '<i class="fas fa-shopping-cart" style="margin-right: 10px;"></i>Beli Sekarang';
    }
});
